  ...
  return (
    <section className="py-20 bg-base-100">
      ...
      <h2 className="text-4xl font-bold text-center mb-3 text-base-content">
        How Our <span className="text-primary">Plateshare</span> Works
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-6 md:px-12 max-w-7xl mx-auto">
        {steps.map((step) => (
          <div
            key={step.id}
            className="bg-base-200 border border-base-300 rounded-2xl p-6 shadow-lg
                       backdrop-blur-sm relative overflow-hidden"
          >
            
            <div className="absolute inset-0 opacity-20">
              <div className="w-24 h-24 bg-primary bg-opacity-30 rounded-full blur-3xl absolute -top-6 -left-6"></div>
              <div className="w-16 h-16 bg-primary bg-opacity-20 rounded-full blur-xl absolute bottom-4 right-4"></div>
            </div>

          
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-3xl shadow-lg">
              {step.icon}
            </div>
            ...
            <p className="text-base-content opacity-70 text-sm leading-relaxed">{step.desc}</p>
          </div>
        ))}
      </div>
    </section>
